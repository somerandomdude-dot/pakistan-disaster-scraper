...
class NDMAScraper(BaseScraper):
    """
    Scraper for NDMA Advisories and Early Warnings.
    Extracts advisory PDF links and parses them.
    """
    ...
    async def fetch(self) -> Any:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if href.endswith('.pdf'):
                if not href.startswith('http'):
                    # NDMA links sometimes have double slashes like //storage/...
                    if href.startswith('//'):
                        href = f"https://www.ndma.gov.pk{href[1:]}"
                    elif href.startswith('/'):
                        href = f"https://www.ndma.gov.pk{href}"
                    else:
                        href = f"https://www.ndma.gov.pk/{href}"
        ...
        for url in pdf_urls[:5]: # limit to recent 5 to avoid overloading
            try:
                ...
                logger.error(f"Failed to fetch NDMA PDF {url}: {e}")
    ...
    def parse(self, raw_content: Any) -> List[Dict[str, Any]]:
        ...
        for item in raw_content:
            ...
            try:
                ...
                # Use URL filename as ID
                filename = url.split('/')[-1].replace('.pdf', '')
                alert_id = f"NDMA-{filename}"
                
                title = "NDMA Advisory / Alert"
                ...
                logger.error(f"Error parsing NDMA PDF {url}: {e}")
    ...
    def normalize(self, parsed_items: List[Dict[str, Any]]) -> List[AlertCreate]:
        ...
        for item in parsed_items:
            try:
                ...
                if "EXTREME" in text_upper or "RED ALERT" in text_upper:
                    severity = "extreme"
                elif "HIGH" in text_upper or "SEVERE" in text_upper:
                    severity = "high"
                
                # Attempt to determine hazard type from text
                hazard_type = "other"
                if "RAIN" in text_upper or "MONSOON" in text_upper:
                    hazard_type = "weather"
                if "FLOOD" in text_upper:
                    hazard_type = "flood"
                if "EARTHQUAKE" in text_upper or "SEISMIC" in text_upper:
                    hazard_type = "earthquake"
                if "HEAT" in text_upper or "TEMPERATURE" in text_upper:
                    hazard_type = "heatwave"
                
                # Title generation
                title = f"NDMA {hazard_type.capitalize()} Advisory"
                ...
                logger.error(f"Normalization error for NDMA item {item.get('source_alert_id')}: {e}")
