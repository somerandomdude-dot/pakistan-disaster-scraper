...
export default function AlertSummaryBanner({ metrics }: { metrics: SummaryMetrics | null }) {
  ...
  return (
    <div className="bg-panel border-b border-slate-200 dark:border-slate-700/80 py-3 px-4 sm:px-6 lg:px-8">
      <div className="max-w-[1440px] mx-auto flex flex-wrap gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-6 flex-wrap text-xs">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[10px] text-text-secondary font-semibold uppercase tracking-[.1em]">Active alerts</p>
              <div className="flex items-baseline gap-2">
                <span className="text-base font-bold text-text-primary">{metrics.active_alerts_count}</span>
          ...
          <div className="h-6 w-px bg-slate-200 dark:bg-slate-700/60 hidden sm:block"></div>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[10px] text-text-secondary font-semibold uppercase tracking-wider">Affected Districts</p>
              <span className="text-base font-bold text-text-primary">{metrics.affected_districts_count}</span>
            </div>
          </div>

          <div className="h-6 w-px bg-slate-200 dark:bg-slate-700/60 hidden md:block"></div>

          {/* Scraper Source Health */}
          <div className="flex items-center gap-2.5">
            <div className="p-2 bg-slate-100 dark:bg-slate-800/60 rounded-lg">
              <RadioTower className="h-4 w-4 text-text-secondary" />
            </div>
            <div>
              <p className="text-[10px] text-text-secondary font-semibold uppercase tracking-wider">Connected Scrapers</p>
        ...
        {metrics.latest_update_time ? (
          <div className="flex items-center gap-1.5 text-xs text-text-secondary font-medium">
            ...
            <span>Latest update <strong className="text-text-primary font-semibold">{getRelativeTime(metrics.latest_update_time)}</strong></span>
          </div>
        ) : (
