...
export default function RecentAlertsTable({ alerts, onSelectAlert }: RecentAlertsTableProps) {
  ...
  return (
    <div className="bg-panel border border-slate-200 dark:border-slate-700/80 rounded-xl shadow-[0_1px_2px_rgba(15,23,42,.04)] overflow-hidden mt-8">
      
      {/* Table Header */}
      <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap justify-between items-center gap-2">
        <div>
          <h3 className="font-semibold text-text-primary text-sm">Recent advisories</h3>
          <p className="text-xs text-text-secondary mt-0.5">Latest reports from monitored public sources</p>
        </div>
        
        <div className="flex items-center gap-3">
          <button
            onClick={() => setSortOrder(sortOrder === "desc" ? "asc" : "desc")}
            className="text-xs text-text-secondary hover:text-text-primary flex items-center gap-1.5 font-medium bg-panel border border-slate-300 dark:border-slate-600 px-3 py-2 rounded-lg transition-colors"
          >
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="w-full text-left text-xs whitespace-nowrap">
          <thead className="bg-panel border-b border-slate-200 dark:border-slate-700 text-text-secondary font-semibold uppercase tracking-wider text-[10px]">
            <tr>
              ...
              return (
                ...
                >
                  <td className="px-5 py-3 text-text-secondary font-mono">
                    <span className="block text-text-primary font-medium">{formatDate(alert.issued_at)}</span>
                    <span className="text-[10px] text-slate-400">{getRelativeTime(alert.issued_at)}</span>
                  </td>
                  
                  <td className="px-5 py-3 font-semibold text-text-primary max-w-[280px] truncate">
                    {alert.title}
                  </td>
                  ...
                  <td className="px-5 py-3 text-text-secondary max-w-[180px] truncate">
                    {location}
                  </td>
        ...
        {paginatedAlerts.map((alert) => {
          ...
          return (
            <div
              key={alert.id}
              onClick={() => onSelectAlert && onSelectAlert(alert)}
              className="p-4 hover:bg-background transition-colors cursor-pointer space-y-2"
            >
              ...
              <div className="flex items-center text-[11px] text-text-secondary gap-3 flex-wrap pt-1">
      ...
      {/* Pagination Footer */}
      {totalPages > 1 && (
        <div className="px-5 py-3 bg-background border-t border-slate-200 dark:border-slate-700 flex justify-between items-center text-xs text-text-secondary">
          <span>
            Showing page <strong className="text-text-primary">{currentPage}</strong> of {totalPages} ({sortedAlerts.length} total)
          </span>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-2.5 py-1 bg-panel border border-slate-200 dark:border-slate-700 rounded disabled:opacity-50 font-medium hover:bg-slate-100 dark:bg-slate-800/60 transition-colors"
            >
              Previous
            </button>
            <button
