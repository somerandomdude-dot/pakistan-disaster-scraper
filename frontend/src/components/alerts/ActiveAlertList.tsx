...
export function AlertListItem({ alert, onSelect }: AlertListItemProps) {
  ...
  return (
    <div
      ...
      className="block border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-background/80 transition-colors px-4 py-4 group cursor-pointer"
    >
      <div className="flex gap-3">
        <div className="mt-0.5 text-text-secondary group-hover:text-blue-900 transition-colors shrink-0">
          <HazardIcon type={alert.hazard_type} />
        </div>

        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start mb-1 gap-2">
            <h4 className="font-semibold text-text-primary text-xs leading-snug group-hover:text-blue-900 transition-colors line-clamp-2">
              {alert.title}
            </h4>
          ...
          <div className="flex items-center text-[10px] text-text-secondary gap-1.5 flex-wrap">
            <span className="font-medium text-text-secondary bg-slate-100 dark:bg-slate-800/60 px-1.5 py-0.5 rounded">
...
export default function ActiveAlertList({ alerts, onSelectAlert }: ActiveAlertListProps) {
  if (!alerts || alerts.length === 0) {
    return (
      <div className="p-8 text-center text-text-secondary flex flex-col items-center justify-center h-full">
        <Inbox className="h-8 w-8 text-slate-300 mb-2" />
        <p className="font-medium text-xs text-text-secondary">No active alerts available</p>
